...
func playlistACLCommand(cfg *Config) *cobra.Command {
	cmd := &cobra.Command{
		Use:                   "acl",
		Short:                 "Manage Playlist ACL",
	...
	cmd := extAPICommand(
		...
		func(cmd *cobra.Command, args []string, extAPI extapiclientv1.Client) (any, error) {
			...
			mustSelect(cfg.AliasType, map[AliasType]func(){
				OpencastAlias: func() { s = svc.NewOpencastPlaylistACL(extAPI) },
				TalesAlias:    func() { s = svc.NewTalesPlaylistACL(extAPI) },
			})()
